...
This project provides a backend REST API for a gaming platform with real-time chat functionality, user authentication, and game/feedback management. It uses PHP sessions for authentication, role-based access control for admin actions, and integrates a WebSocket server for live chat. Logging is implemented via Monolog.

## Key Features

- **Session-Based Authentication**: Users log in with an identifier (email or nickname) and password. The server maintains a PHP session.
...
- **Logging**: Detailed logs generated by Monolog (stored in `logs/chat.log`) and PHP’s native `error_log`.
...
```
├── api/                   # Individual API endpoint scripts
├── config/                # Configuration (DB connection, session)
├── helpers/               # Helper functions (auth, response, image handling)
├── logs/                  # Application logs (monolog)
├── sessions/              # PHP sessions stored on server
├── src/                   # Application source (e.g. Chat class)
├── uploads/               # Uploaded avatars and images
...
- **Location**: Logs are primarily written to `logs/chat.log` via Monolog in the `Chat` class.
- **Error Logs**: Additional PHP error logs and database errors may be logged using `error_log()` calls.
- **Granularity**: Info-level and error-level logs help diagnose issues with chat, database connectivity, and operations.

## API Endpoints Detail

Below is an overview of the main API endpoints, their methods, parameters, and typical responses. All endpoints return JSON.
...
>>>>>>> c180107e44a230ecfb20a895be5224aa9eccb77d
...
4. **Run the WebSocket server**:
   ...
   ```
5. **Access the API endpoints** via a web server (e.g., Apache or Nginx) pointing to the `api/` directory.

## Dependencies

- **cboden/ratchet**: WebSocket server
- **monolog/monolog**: Logging
- **PHP PDO extension**: MySQL database interaction

## Security and Validation

- All user input is validated or sanitized before database operations.
- Session-based authentication ensures only logged-in users can perform protected actions.
- Admin actions are restricted to users with `user_role = 'admin'`.
- Avatar and image uploads have size and dimension checks.
